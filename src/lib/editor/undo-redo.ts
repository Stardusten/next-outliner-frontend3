...
import { toBlock } from "../blocks/utils";
import type {
  BlockStorage,
  BlockTransaction,
...
export class UndoRedoManager {
  ...
  addItem(item: StackItem) {
    ...
    for (const tx of item) {
      const isUndo = tx.source === UpdateSources.undo(this.editorId);
      const isRedo = tx.source === UpdateSources.redo(this.editorId);
  ...
  private doInverse(
    ...
    for (let i = stackItem.length - 1; i >= 0; i--) {
      ...
      inverseTx.source =
        operation === "undo"
          ? UpdateSources.undo(this.editorId)
          : UpdateSources.redo(this.editorId);

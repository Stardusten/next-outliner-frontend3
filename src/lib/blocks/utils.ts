...
export function sortChildren(block: BlockLoaded) {
  block.update((val) => {
    val.childrenBlocks.sort(
      (a, b) => a.get().fractionalIndex - b.get().fractionalIndex
    );
  });
}
...
export function toBlock(block: BlockLoaded): Block {
  const { id, type, folded, parentId, fractionalIndex, content } = block.get();
  return { id, type, folded, parentId, fractionalIndex, content };
}

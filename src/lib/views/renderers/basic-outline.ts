import { getBlockNode, getRootBlockIds } from "@/lib/app/block-manage";
  ...
  if (!blockData.folded && !rootOnly) {
    if (res instanceof Error) {
      ...
      for (const id of res as BlockId[]) {
        const blockNode = getBlockNode(editor.app, id);
        if (blockNode == null) {
...
export function renderOutline(editor: TiptapEditorView): Node {
  if (!editor.tiptap) throw new Error("tiptap no init");
  const schema = editor.tiptap.schema;
  ...
  for (const rootBlockId of rootBlockIds) {
    const blockNode = getBlockNode(editor.app, rootBlockId);
    if (blockNode == null) {

...
import type { CompletionStatus, TiptapEditorViewEvents } from "../editor";
...
export function executeCompletion(blockId: string, view: EditorView): boolean {
  ...
  const blockRefNode = schema.nodes.blockRef.create({
    blockId,
    isTag: isTag || false,
  });
...
export function createCompletionHelperPlugin(
  emitter: Emitter<TiptapEditorViewEvents>
) {
